...
This was implemented in python with the ``elliptical'' and ``Tavg'' functions
in Appendix \ref{apx:analytical-np}. The elliptical integrals are numerically evaluated using a quadrature method.
...
\(C_1 \ln(t) + C_2\) to equation \ref{eq:tavg}, in the ``kmeas'' function, also
in Appendix \ref{apx:analytical-np}.

Then, all that is left is to evaluate the functions at various combinations of
\(k_xy\), \(k_z\) and \(\theta\), which is done with the ``\_\_main\_\_'' procedure
in Appendix \ref{apx:analytical-np}. After iterating through all the values,
this procedure prints to stdout in .csv format, so that a table of values may be generated and piped to a text file for future analysis.
...
expected cmonductivity measurement given the theory holds for anisotropic
materials.
