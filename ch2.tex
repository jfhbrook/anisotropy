...
This was implemented in python with the following functions:
...
Note that the elliptical integrals are numerically evaluated.
...
\(C_1 \ln(t) + C_2\) to equation \ref{eq:tavg}. In particular, the following
python function was defined:
...
``proj(matrix, rotation)'' implements equation \ref{eq:projection}, where 
``matrix'' and ``rotation'' are used to construct \([K]\). After iterating
through all the values, this procedure prints to stdout in .csv format, so that
a table of values may be generated and piped to a text file for future analysis.
